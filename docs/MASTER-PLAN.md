...
| echo-agent 全 workspace 语义治理 | Baseline complete / Finding remediation ongoing | PR #117交付workspace baseline、Audit、SDK scope reset与首批repair，PR #119修复squash-aware ancestry，PR #121交付生命周期结算Wave 1，PR #123闭合Plugin component isolation，PR #124以`29cea08c`闭合Journal/checkpoint generation与Workflow四入口执行权威。当前9724个canonical identity中5622 external contract、1774 Host/Rust-only、790 language intrinsic、90 internal helper、1448 deferred；95个Finding中44 resolved、51 open并一对一映射GitHub Issue，SDK仓库拆分另由#122协调。Wave 2本地`verify.sh`、17个独立feature、三语言SDK、semantic strict/change-evidence与独立review全部通过，PR #124远端10/10 CI成功。路线见 [`delivery map`](./supreme/plans/2026-09-13T1448-echo-agent全workspace语义治理/delivery-map.md) 与 [`完整优化清单`](./2026-09-14-echo-agent-semantic-governance-findings.md)。 |

## 当前基线

| 仓库 | 本地基线 | 远端/发布状态 |
| --- | --- | --- |
| `echo-agent` | `29cea08c` (`main`) | PR #124已squash merge且GitHub签名verified；本地/远端main一致。95个Finding中44个resolved Issue已关闭，51个open Issue保持后续backlog。 |
...
| superproject | `doc/Echoyue/semantic-wave2-summary` | 本次只交付最新语义治理总结、路线更新和`echo-agent@29cea08c`指针；`echo-agent-cli`、`echo-website`及其它线程工作树改动不进入本次提交。 |
...
### echo-agent 全 workspace 语义治理（Baseline Complete / Remediation Ongoing，2026-09-16）

SDK identity inventory保留为漂移监控和独立backlog，不再用其完成比例表示项目进度，也不再逐identity建PR。全workspace discovery、高风险审计、SDK scope reset、首批Finding repair、concept convergence和squash-aware baseline门禁已进入远端main；PR #121闭合18个终态、恢复、权限、协议与副作用Finding，PR #123闭合Plugin component isolation，PR #124在`29cea08c`闭合Journal/checkpoint generation identity与Workflow四入口执行权威。95个Finding全部映射唯一GitHub Issue，44个resolved Issue已随交付关闭，剩余51个继续按风险和Finding独立推进。

下一修复前沿优先交付K8s cleanup owner（#62）和Trace/Audit persistence visibility（#46）候选；Scheduler durable occurrence（#84）需先闭合EKO stable data-root consumer与SDK拆分依赖，Agent adapter close（#36）已完成ACP子边界但仍等待其它adapter生命周期。随后处理SDK gap ACK replay watermark（#120）、MCP/LSP transport close（#55）及其它持久化/cleanup owner。`echo-sdk-host`与`echo-sdk-protocol`独立仓库迁移线程必须吸收`29cea08c`的Turn delivery、checkpoint settlement、Journal identity、gap generation和SDK scope变化；迁移稳定前不在本线程无必要扩大这两个crate。

统一Tool Surface / ToolRouter草案的首次复审结论为block：surface必须按model request/iteration generation冻结，必须复用现有`ToolInvocation`/`ToolRuntime`/`ToolExecutionContext`，execution traits不得取代动态Permission与现有ToolManager/network生命周期。草案在修订和复审通过前不是正式ADR，不得进入实现。
