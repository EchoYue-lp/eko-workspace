...
- **放框架的条件**(全部满足):这是**任何**用 echo-agent 的 agent 都可能需要的通用能力(不依赖 EKO 的产品决策)、与具体业务/产品形态无关、可以独立编译、测试和文档化。它不要求已经存在第二个外部消费者；“多个项目可复用”是语义判断，不是等待消费者出现的前置门槛。
...
- **外部采用量不是资格门槛**:框架本来就是为了吸引新的开发者。没有现有外部调用不能作为拒绝产品无关能力进入 framework 的理由；它只能作为 API 稳定性、文档优先级和是否拆包的补充证据。
...
1. **先写分层判定**:明确列出“通用机制 / EKO 产品策略 / 适配边界”三部分。依赖 DAG、重试、取消、revision safe point 等跨产品成立的机制放框架;DomainProfile、reviewer 策略、worktree、文件权威、UI/TUI/CLI 投影等留应用。没有第二消费者不能阻止通用机制下沉；拿不准时先留应用,不得把 EKO 字段包装成“通用配置”后塞进框架。
...
1. **`echo-agent/`、`echo-agent-cli/` 只承载正式、长期维护的框架/产品文档。** 通用框架概念、公共 API 和框架架构文档归 `echo-agent/`；EKO 产品架构、运行时、交互与应用配置文档归 `echo-agent-cli/`。正式文档必须放在其所属子仓库内，并随该子仓库代码独立提交、维护和发布。
2. **顶层 `lp-agent/` 只承载跨仓库协作内容和阶段性工程材料。** 允许保存跨仓库协作规则，以及测试记录、spec、计划、审计报告、评审材料和阶段验收证据；不得把正式的框架/产品说明文档放在顶层 `docs/`，也不得让顶层文档成为子仓库产品行为或公共 API 的长期事实源。

3. **任何代码修改都必须同步检查并修改对应的文档、`examples` 代码示例以及 `echo-website` 相关内容**,确保实现、示例和对外说明始终一致。
4. **凡是涉及项目架构、关键组件或重点代码的新增与修改，都必须补充或更新对应文档和代码注释**。注释应重点说明设计意图、关键约束和非显而易见的逻辑，不写只是复述代码表面行为的无效注释。
5. **提交前必须明确检查文档、`examples` 和 `echo-website` 是否需要同步更新**。某项与本次改动无关或确实不适用时，必须在提交说明、PR 描述或任务交付说明中写明原因，不得静默跳过。
6. **架构变更必须使用 ADR(Architecture Decision Record)记录**。ADR 至少应包含背景、候选方案、最终决策、取舍理由和影响范围；优先遵循所属子仓库已有的 ADR 约定，没有现有约定时放入该子仓库的 `docs/adr/` 目录。
7. **`examples` 必须纳入可执行的编译或测试链路**，并作为提交前验证的一部分，防止示例与真实 API 长期漂移。新增或修改示例时，必须同步补齐对应的编译或测试入口。
